...
library("pheatmap")
sampleDists <- dist(t(DMSO_imatinib))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- paste(colnames(DMSO_imatinib))
colnames(sampleDistMatrix) <- paste(colnames(DMSO_imatinib))
...
tiff(filename = "DMSO_imatinib_sample_distance_plot.tif",width = 12,height = 8,units ="cm",compression="lzw",bg="white",res=1024)
sample_distance_plot
dev.off()

tiff(filename = "DMSO_imatinib_PCA_plot.tif",width = 12,height = 8,units ="cm",compression="lzw",bg="white",res=1024)
PCA_plot
dev.off()
